...
function ClassCard({
  ...
  classItem: WithId<RecordedClass>;
  ...
  const CardWrapper = ({ children }: { children: React.ReactNode }) =>
    classItem.isFree ? (
      <a
        ...
        className="block hover:shadow-lg transition-shadow duration-300 h-full group"
      >
        {children}
      </a>
    ) : (
      <Link
        href="/billing"
        className="relative h-full block hover:shadow-lg transition-shadow duration-300 group"
      >
        {children}
      </Link>
    );
...
export function ClassesPageClient({
  ...
  recordedClasses: WithId<RecordedClass>[];
  isInstructor: boolean;
}) {
      ...
      <Tabs defaultValue="all" className="w-full">
        ...
        <TabsContent value="all">
          {isLoading ? (
            ...
            <ClassGrid classes={recordedClasses || []} />
          )}
        </TabsContent>
        ...
        <TabsContent value="premium">
          {isLoading ? (
            ...
            <ClassGrid classes={premiumClasses} />
          )}
        </TabsContent>
      </Tabs>
    </div>
